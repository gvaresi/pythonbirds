...
class Passaro(Ator):
    ...
    def calcular_posicao(self, tempo):
        ...
        if self._esta_voando():
            ...
            self._calcular_posicao_horisontal(delta_t)
    ...
    def _calcular_posicao_vertical(self, delta_t):
        ...
        angulo_radianus = self._angulo_de_lancamento
        y_atual += self.velocidade_escalar * delta_t * math.sinh(angulo_radianus)
        y_atual -= (GRAVIDADE *(delta_t ** 2)) / 2
        self.y = y_atual

    def _calcular_posicao_horisontal(self, delta_t):
        x_atual = self._x_inicial
        angulo_radianus = self._angulo_de_lancamento
        x_atual += self.velocidade_escalar * delta_t * math.cos(angulo_radianus)
        self.x = x_atual
